...
if __name__ == '__main__':
    ...
    # graph_mode = 'knn'
    for k in [4, 6, 10, 16]:
        adjacency_raw, features = load_data(f'data/k_{k}.gml', 'data/features.npy')
    ...
    # plt.ylim((0, 1))
    plt.legend(['k=4', 'k=6', 'k=10', 'k=16'])
    plt.savefig('modularity_k.jpg', dpi=300)
    plt.show()

    # plt.figure(figsize=(24, 18))

    df = pd.read_csv('data/data.csv')
    dic = {0: 4, 1: 6, 2: 10, 3: 16}
    for i in range(len(clusters_list)):
        ...
        plt.savefig(f'k_result_{dic[i]}.jpg', dpi=200)
