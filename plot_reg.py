...
if __name__ == '__main__':
    ...
    mod_list = []
    clusters_list = []
    adjacency_raw, features = load_data(f'data/k_10.gml', 'data/features.npy')
    ...
    for reg in [0, 1, 10, 100]:
        model = MyModel('gat', num_channels, num_clusters, reg)
    ...
    # plt.ylim((0, 1))
    plt.legend(['reg = 0', 'reg = 1', 'reg = 10', 'reg = 100'])
    plt.savefig('modularity_reg.jpg', dpi=300)
    plt.show()

    # plt.figure(figsize=(24, 18))

    df = pd.read_csv('data/data.csv')
    dic = {0: 0, 1: 1, 2: 10, 3: 100}
    for i in range(len(clusters_list)):
        plt.figure()
        plt.scatter(df['X'], df['Y'], c=clusters_list[i], s=1)
        plt.xticks([])
        plt.yticks([])
        plt.savefig(f'reg_result_{dic[i]}.jpg', dpi=200)
